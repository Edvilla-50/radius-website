...
<div class="flex flex-col h-screen">
    ...
    <div class="flex items-center justify-between px-6 py-4 bg-gray-800 shadow">
        ...
        <div class="flex gap-3">
            ...
            <button id="templateMinimal"
                    class="px-3 py-1 bg-gray-700 rounded hover:bg-gray-600 text-sm">
                Minimal
            </button>

            <button id="templateDark"
                    class="px-3 py-1 bg-gray-700 rounded hover:bg-gray-600 text-sm">
                Dark
            </button>

            <button id="templateGamer"
                    class="px-3 py-1 bg-gray-700 rounded hover:bg-gray-600 text-sm">
                Gamer
            </button>
...
<div id="musicModal"
    ...
    <div class="bg-gray-800 rounded-xl p-6 w-80 shadow-2xl space-y-3">
        ...
        <div id="musicStep2"
            ...
            <button id="musicDisconnectBtn"
                    class="w-full py-2 bg-red-600 hover:bg-red-500 text-sm font-semibold rounded-lg transition">
...
(function () {
    ...
    async function checkExistingConnection() {
        ...
        try {
            ...
            const res =
                await fetch(`${BACKEND}/api/music/${USER_ID}`);
    ...
    async function initMusicKit() {
        ...
        const tokenRes =
            await fetch(`${BACKEND}/api/music/dev-token`);
        ...
        if (!tokenData.token) {
            throw new Error("Developer token was missing from response");
        }
    ...
    musicConnectBtn.addEventListener("click", async () => {
        ...
        try {
            ...
            const res =
                await fetch(`${BACKEND}/api/music/connect/${USER_ID}`, {
            ...
            if (!res.ok) {
                ...
                throw new Error(
                    `Backend connect failed: HTTP ${res.status} ${message}`
                );
    ...
    musicDisconnectBtn.addEventListener("click", async () => {
        ...
        try {
            ...
            const res =
                await fetch(`${BACKEND}/api/music/disconnect/${USER_ID}`, {
    ...
    function showPreview(snippet) {
        ...
        document.getElementById("musicPreviewArt").src =
            snippet.albumArtUrl || "";

        document.getElementById("musicPreviewTrack").textContent =
            snippet.trackName || "No recent tracks yet";

        document.getElementById("musicPreviewArtist").textContent =
            snippet.artistName || "";
    ...
    musicInsertBtn.addEventListener("click", () => {
        ...
        const widgetHtml = `<!-- Radius: Recently Played (auto-updates) -->
...
(function() {
  ...
  function loadRecentlyPlayed() {
    ...
    fetch("https://www.radius-create.com/music-proxy.php?id=${uid}&t=" + Date.now(), {
      cache: "no-store"
    })
    .then(function(r) {
    ...
    .then(function(data) {
      ...
      document.getElementById("rp-art-${uid}").src =
          data.albumArtUrl || "";

      document.getElementById("rp-track-${uid}").textContent =
          data.trackName;

      document.getElementById("rp-artist-${uid}").textContent =
          data.artistName || "";
    ...
    .catch(function(error) {
      ...
      console.error(
          "Recently Played refresh failed:",
          error
      );
  ...
  btn.onclick = function() {
    ...
    audio.play()
      ...
      .catch(function(error) {
        ...
        console.error(
            "Recently Played preview failed:",
            error
        );
...
<\/script>`.trim();
        ...
        const cm =
            cmElement && cmElement.CodeMirror;
